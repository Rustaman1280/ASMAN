...
@section('content')
<div class="max-w-4xl mx-auto" x-data="barangEditForm()">
...
<script>
function barangEditForm() {
    // Pre-fill from existing pivot data
    let initialLokasi = [];
    @if(old('lokasi'))
        initialLokasi = @json(old('lokasi'));
    @else
        initialLokasi = @json($barang->ruangans->map(fn($r) => ['ruangan_id' => (string) $r->id, 'jumlah' => $r->pivot->jumlah])->values());
    @endif
    ...
    return {
        ...
        jumlahBaik: {{ old('jumlah_baik', $barang->jumlah_baik ?? 0) }},
        jumlahRusakRingan: {{ old('jumlah_rusak_ringan', $barang->jumlah_rusak_ringan ?? 0) }},
        jumlahRusakBerat: {{ old('jumlah_rusak_berat', $barang->jumlah_rusak_berat ?? 0) }},
        ...
        init() {
            ...
            // Initial sync ONLY IF it's newly edited value or needs adjustment,
            // but for edit, we already have initial values, sync might overwrite them if mismatch,
            // but let's assume totalLokasi should always be respected:
            this.syncBaik(); 
        },
