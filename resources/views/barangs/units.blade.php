...
<div class="max-w-5xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('barangs.edit', $barang) }}" class="inline-flex items-center px-4 py-2 border border-slate-200 rounded-xl shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50">
                Edit Jumlah Utama
            </a>
        </div>
    </div>
                ...
                <div x-data="{ isEditing: false }" class="bg-white border rounded-xl p-4 shadow-sm flex flex-col hover:shadow-md transition-shadow {{ $unit->kondisi == 'baik' ? 'border-emerald-200' : ($unit->kondisi == 'rusak_ringan' ? 'border-amber-200' : 'border-rose-200') }}">
                    ...
                    <div x-show="!isEditing" class="flex flex-col flex-grow">
                        ...
                        <div class="pt-3 border-t border-slate-100 flex justify-end">
                            <button type="button" @click="isEditing = true" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Edit Catatan</button>
                        </div>
                    </div>
