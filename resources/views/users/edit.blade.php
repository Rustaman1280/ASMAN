...
<div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="p-6">
        <form action="{{ route('users.update', $user) }}" method="POST">
            ...
            <div class="mb-6 {{ old('role', $user->role) == 'pj_ruangan' ? '' : 'hidden' }}" id="ruangan_container">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto p-3 border border-slate-200 rounded-lg bg-slate-50">
                    @forelse($ruangans as $ruangan)
                        <label class="flex items-start space-x-3 cursor-pointer p-2 hover:bg-slate-100 rounded transition-colors">
                            <input type="checkbox" name="ruangans[]" value="{{ $ruangan->id }}" class="mt-1 w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" {{ (is_array(old('ruangans')) && in_array($ruangan->id, old('ruangans'))) || (!old('ruangans') && $user->ruangans->contains($ruangan->id)) ? 'checked' : '' }}>
