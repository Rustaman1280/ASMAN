...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center">
        ...
        <h3 class="text-lg font-semibold text-slate-700">Edit Data Unit</h3>
    </div>
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="barang_id" id="barang_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" required>
                            ...
                            @foreach($barangs as $barang)
                                <option value="{{ $barang->id }}" {{ old('barang_id', $unit->barang_id) == $barang->id ? 'selected' : '' }}>{{ $barang->nama_barang }} ({{ $barang->kode_barang }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_unit" id="kode_unit" value="{{ old('kode_unit', $unit->kode_unit) }}" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400" placeholder="Contoh: UNIT-001" required>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            @php
                                $currentType = old('lokasi_type', $unit->lokasi_type === 'App\Models\Kelas' ? 'kelas' : 'lab');
                            @endphp
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="kondisi" id="kondisi" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" required>
                            ...
                            <option value="hilang" {{ old('kondisi', $unit->kondisi) === 'hilang' ? 'selected' : '' }}>Hilang</option>
                        </select>
                ...
                <div>
                    ...
                    <textarea name="detail_unit" id="detail_unit" rows="3" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400" placeholder="Keterangan tambahan tentang unit ini...">{{ old('detail_unit', $unit->detail_unit) }}</textarea>
...
<script>
    ...
    const currentLokasiId = "{{ old('lokasi_id', $unit->lokasi_id) }}";
