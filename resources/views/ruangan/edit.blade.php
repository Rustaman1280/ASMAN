...
@section('content')
<div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm max-w-3xl mx-auto" x-data="ruanganForm('{{ $ruangan->kategori }}', '{{ $ruangan->jenis_ruangan }}')">
    <div class="p-6 border-b border-slate-100">
        <h3 class="text-lg font-semibold text-slate-800">Form Edit Ruangan</h3>
    </div>
    <form action="{{ route('ruangans.update', $ruangan) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama', $ruangan->nama) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" required>
            ...
            <div x-show="needsTingkat">
                ...
                <input type="text" name="tingkat" value="{{ old('tingkat', $ruangan->tingkat) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all">
            ...
            <div x-show="needsJurusan">
                ...
                <select name="jurusan_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all">
                    ...
                    @foreach($jurusans as $j)
                        <option value="{{ $j->id }}" {{ old('jurusan_id', $ruangan->jurusan_id) == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                    @endforeach
                </select>
        ...
        <div class="mt-8 flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-sm transition-colors cursor-pointer">Simpan Perubahan</button>
        </div>
    </form>
</div>

<script>
function ruanganForm(initKategori, initJenis) {
    const cats = {
